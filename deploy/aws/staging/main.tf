...
resource "aws_security_group" "game" {
  name        = var.name
  description = "Private Farfield staging: outbound Tunnel and SSM only; no ingress"
  vpc_id      = aws_vpc.game.id
  egress {
    from_port   = 0
    to_port     = 0
...
resource "aws_iam_role_policy" "private_configuration" {
  name = "farfield-staging-private-configuration"
  role = aws_iam_role.game.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      ...
      Resource = [
        "arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter/farfield/staging/friend-rpc-url",
        "arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter/farfield/staging/tunnel-token"
      ]
    }]
  })
}
...
resource "aws_instance" "game" {
  ...
  root_block_device {
    ...
    tags                  = { Name = "${var.name}-data", Project = "Farfield", Environment = "staging" }
  }
  tags = { Name = var.name }
  lifecycle {
