...
Daily private room backups are scheduled for 04:00 UTC with up to five minutes of jitter. Local match checkpoints run every five seconds and during graceful shutdown.  The first backup succeeded at 12:20 UTC; its S3 object was confirmed encrypted and versioned. The live snapshot has mode `0600` and UID/GID `1000:1000`. Farfield, Docker, SSM and the backup timer were all active after deployment.
...
## Touch construction and workforce economy release

Application release `92b08188891a75db1c27ed8f3eb325ce86781ebf`, browser build `c3fad9100bb38e60`, fixes immediate touch previews and dragging, on-site infirmary staffing, own wreckage clearance/rebuilding and floor shadow order. Workforce growth now requires sustainable farming, uses a refundable six-second recruitment queue, and applies gradual food-shortage penalties. Source [PR #2](https://github.com/b00ste/farfield/pull/2) is merged. Both [push CI](https://github.com/b00ste/farfield/actions/runs/35619742773) and [PR CI](https://github.com/b00ste/farfield/actions/runs/35619747121) passed all 120 tests, four-seat streams and process/container recovery.

Runtime image: `sha256:fed6118a440e16f7bbce734455bfac452d8f49983a85612b0998d68038dbc234`. Strict public HTTPS, API and containers passed health checks. The existing room and human seat were retained across deployment. Safe comparisons confirmed the private RPC setting was preserved; runtime and snapshot files remain mode `0600`, and the post-release backup succeeded. Existing Farfield resources were reused; no infrastructure or Phlox changes. Sanitized evidence: `artifacts/aws-release-92b08188.json`.

Public browser checks asserted this exact build and passed real-time tablet recruitment, cancellation/refund, same-seat reload, food shortage and recovery, 320px layout bounds and touch/pen placement, rotation and pinch safety. Zero page errors; both test rooms were left afterward. Reports: `artifacts/economy-tablet-production.json`, `artifacts/touch-placement-production.json` and `artifacts/touch-economy-release-production.json`. Full-window touch passed; the optional scaled submission HUD has a separately tracked Chromium touch-click coordinate limitation. Physical Safari remains a device check.
