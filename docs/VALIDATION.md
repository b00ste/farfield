...
## PWA installation and contextual HUD — 21 September

The title menu now offers Install game without requiring a wallet. Browsers supporting installation prompts use their native prompt; iPhone/iPad and Safari receive short platform instructions. Standalone detection hides the action in the installed app. The manifest includes normal/maskable PNG icons and the host includes Apple touch metadata. Offline launches show a reconnect screen; only six public icon/offline files enter the service-worker cache. No API, wallet, RPC, stream or game bundle is cached. Updates never request immediate activation or force a reload.

Candidate Chrome checks passed at 320×568, 390×844, 568×320, 852×393 and 1024×768, with connected/disconnected menus fitting the screen and install/menu targets at least 44px. A real persistent Chrome profile reported no installability errors. Actual offline fallback and cache contents passed. During a live match, a second build produced a genuinely new worker which remained waiting while the same player's clock and SSE continued; no forced reload or API caching occurred. Synthetic install events cover dismissal, retry, acceptance and hiding after appinstalled; synthetic iOS/standalone signals cover platform instructions and installed presentation. These are not native OS installation tests.

The suite found and fixed two issues before release: repeated SDK builds rejected our additional PWA output files (now staged separately), and privacy-blocked localStorage crashed wallet dependencies (now uses page-only memory with guarded wallet persistence). Denied-storage menu and installation help pass. Existing RainbowKit header wallet buttons remain 40px high; title-menu targets are at least 44px.

All 96 logic/server tests, TypeScript, FriendSDK validation and repeated production builds pass. Container smoke now checks manifest MIME, PNG sizes, offline route and versioned uncached worker packaging. Native iPhone/iPad/Safari installation and external-wallet handoff remain on the physical-device checklist.

The compact HUD checks passed at 1440×900 and 390×844: both worker controls share the same group glyph; unfinished construction shows a hammer and completed Foundry work shows a pickaxe; recruitment has no visible cost caption while its accessible cost remains. Additional 320px portrait and 852px landscape checks exercised Bottom/Left/Right settings and camera-hint bounds/nonoverlap; narrow portrait intentionally hides the hint. Desktop and phone screenshots were reviewed. Gameplay commands and progress used the real candidate server with wallet/NFT fixtures. Evidence: `artifacts/pwa-candidate.json`, `artifacts/compact-hud-candidate.json` and representative PWA/HUD screenshots.

Deployed to the public AWS game as source `e7aa83ee81726db54edabf56239b537ad75a85e2`, build `f997b7f56218d829`. Both release CI runs passed, including the packaged PWA assets. Public-origin PWA and compact HUD suites passed again. Unmocked FriendSDK discovery returned 42 eligible Friends plus one hidden Friend, with working artwork, sanitized provider failures and no private endpoint leakage or direct provider requests. Production update checks do not rebuild the server; the genuinely changed-worker waiting test was performed on the isolated candidate. Reports: `artifacts/pwa-production.json`, `artifacts/compact-hud-production.json` and `artifacts/production-rpc-pwa-release.json`.

## Zerion and MetaMask direct entries — 21 September

`host/wallet-config.ts` now lists Browser Wallet, Zerion, MetaMask and WalletConnect using RainbowKit's built-in connectors. The configured Rainbow entry is removed. The navy/sage wallet theme remains unchanged.

Typecheck and build pass. Both [push CI](https://github.com/b00ste/farfield/actions/runs/35605746091) and [PR CI](https://github.com/b00ste/farfield/actions/runs/35605752343) passed for source `6ac15964435a99135db5964d86b56d40aca4b9a6`. The bounded `tests/wallet-options-browser.mjs` check passed six candidate cases: desktop, simulated iPad standalone and simulated iPhone, each selecting Zerion and MetaMask. Desktop produced real unpaired QR codes; mobile produced real `zerion://wc` and `metamask://connect` links with pairing parameters. Reports retain only protocol/host/path and booleans, never pairing URIs. No wallet approval, signing or funds were involved. Native OS launch and authorization remain physical-device checks.

The first MetaMask mobile test missed the `metamask:` protocol and incorrectly reported a stalled connector. Expanded navigation capture confirmed the standard SDK path works; no application workaround was retained. Evidence: `artifacts/wallet-options-candidate.json` and wallet-options phone/tablet screenshots. Run the script only in Browser Testing, with `TEST_URL` selecting a candidate or the public site.

The same six-case matrix passed on `https://farfield.fun` after deployment of build `f8a6f994086d2a2a`, with no page errors. Emulated mobile Chromium emitted a user-gesture warning on MetaMask native-launch attempts; link generation passed, but OS launch/return and Safari approval are not certified by this check. Zerion link generation had no such warning. Public evidence: `artifacts/wallet-options-production.json`. The owner should fully reopen the installed game and try the direct Zerion/MetaMask choices on the physical device.
